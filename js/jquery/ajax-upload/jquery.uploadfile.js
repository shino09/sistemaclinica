    ...
    $.fn.uploadFile = function (options) {
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            $(document).on('dragover', function (e) {
                e.stopPropagation();
                e.preventDefault();
                ddObj.css('border', '1px solid #cccccc');
            });
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for (var i = 0; i < files.length; i++) {
                if (!isFileTypeAllowed(obj, s, files[i].name)) {
					if (s.showError){
						noty({
							text:  "<b>" + files[i].name + "</b> " + s.extErrorStr + s.allowedTypes,
							layout: 'topCenter',
							type: 'error',
							killer: true
						});
                ...
                if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if (s.showError){
						noty({
							text:  "<b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize),
							layout: 'topCenter',
							type: 'error',
							killer: true
						});
            ...
            fileInput.change(function () {
                ...
                if (this.files) //support reading files
                {
                    ...
                    if (!isFileTypeAllowed(obj, s, filenameStr)) {
						if (s.showError){
							noty({
								text:  filenameStr + s.extErrorStr + s.allowedTypes,
								layout: 'topCenter',
								type: 'error',
								killer: true
							});
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj) {
            ...
            var options = {
                ...
                beforeSend: function (xhr, o) {
                    ...
                    pd.progressDiv.css("display","inline-block");
                    pd.cancel.hide();
                    pd.done.hide();
                    if (s.showAbort) {
                        pd.abort.show();
