	...
	function RestarHoras($horaini,$horafin)
{
	...
	/*function formatearHora($hora, $despliegue = true){
		if($despliegue):
			return number_format($hora,0,'','.');
		else:
			$hora = trim($hora);
		...
		function formatearFecha3($fecha, $despliegue=false, $separador='-'){
						echo 'llego a la funcion formatear';
		...
		function formatearFecha2($fecha, $despliegue=false, $separador='-'){
		$fecha = str_replace("/","-",$fecha);
		if(!$fecha)
			return "Sin dato.";
		if($despliegue):
			$new_fecha = explode(' ',$fecha);
			$new_fecha = explode('-',$new_fecha[0]);
			return $new_fecha[2].$separador.$new_fecha[0].$separador.$new_fecha[1];
		else:
			$new_fecha = explode('-',$fecha);
			return $new_fecha[2].$separador.$new_fecha[0].$separador.$new_fecha[1];
		endif;
	...
	function formatearValor($valor, $despliegue = true){
		...
		function formatearValor2($valor, $despliegue = true){
		if($despliegue):
			return number_format($valor,0,'','.');
		else:
			$valor = trim($valor);
	...
	########################## files #############################
	function crear_directorio($ruta,$ftp = false){
		if(!is_array($ruta)){
			$rutas = array($ruta);
		}else{
			$rutas = $ruta;
