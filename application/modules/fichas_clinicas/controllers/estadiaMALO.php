...
class Estadia extends CI_Controller {
	...
	public function index($codigo = false)	{
		...
		if($this->uri->segment(3)=='fecha' && $this->uri->segment(4) !='all'){
			
			if ($url_busqueda == false){
			$url_busqueda = 'fecha/'.$this->uri->segment(4).'/';
			$data['fecha'] = $this->uri->segment(4);
			$where .= $this->prefijo_pri."fecha_ingreso = '".$this->uri->segment(4)."'";
			...
			$url_busqueda = 'fecha/'.$this->uri->segment(4).'/';
			$data['fecha'] = $this->uri->segment(4);
			$where .= " and ".$this->prefijo_pri."fecha_ingreso = '".$this->uri->segment(4)."'";
			$config['uri_segment'] = $segment = 9;
			$joinfecha=1;
			}
		}
		...
		$data = false;
		if($codigo)
			$data["datos"] = $this->ws->obtener($this->id_modulo,$this->prefijo_pri."codigo = {$codigo}");
